...
def sum_until_prime_input():
    ...
    Sum=0
    while True:
        num=int(input('Enter an integer that is greater than 2:'))
        Sum=Sum+num
        if is_prime(num)==True:
            break
    print('The sum of the integers input is:',Sum)
...
def prime_gap(m):
    ...
    # -------------------------------------------------------------------------
    count=2
    while next_prime(count+1)-count<m:
        count=next_prime(count+1)
    return count
